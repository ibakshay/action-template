...
function commentContent(signed, committerMap) {
    ...
    let text = `**CLA Assistant Lite:** <br/>Thank you for your submission, we really appreciate it. Like many open source projects, we ask that ${you} sign our [Contributor License Agreement](${url_1.pathToCLADocument()}) before we can accept your contribution.
    You can sign the CLA by just  posting a Pull Request Comment same as the below format. 
    
    **I have read the CLA Document and I hereby sign the CLA** <br/>`;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!prComment) {
                ...
                const reactedCommitters = yield reaction_1.default(prComment.id, committerMap, committers);
                ...
                core.debug("test--------------------->" + JSON.stringify(committerMap.notSigned));
                yield octokit_1.default.issues.updateComment({
                    owner: github_1.context.repo.owner,
                    repo: github_1.context.repo.repo,
                    comment_id: prComment.id,
